...
exports.getCentros = async (req, res, next) => {
  ...
  if (
    ...
    var expirado = comprobarToken.compruebaToken(
      jwt_decode(req.headers["authorization"] /* { header: true } */)
    );
        ...
        try {
          ...
          const result = Centro.updateCentro(req.body, user)
            ...
            .catch(function () {
              res
                .status(401)
                .json({ errors: "no se ha podido actualizar el centro" });
            });
        } catch (err) {
          res.status(500).json({ error: err });
        }
...
exports.deleteUserAndFPByCentro = async (req, res, next) => {
  if (
    ...
    if (expirado) {
      ...
      try {
        const user = jwt_decode(req.headers["authorization"]).sub;
        const centro = await Centro.deleteUserAndFPByCentro(
          req.params.codigoCentro,
          user
        );
    ...
    if (expirado) {
      ...
      if (!errors.isEmpty()) {
        ...
        try {
          ...
          const result = Centro.createCentro(req.body, user)
            .then(function (result) {
              console.log("Promise Resolved");
            ...
            .catch(function () {
              res
                .status(401)
                .json({ message: "no se ha podido crear el centro:" + err });
            });
        } catch (err) {
          res.status(500).json({ error: err });
        }
